...
Отправка:
  - **Запрос преподписанного URL:** Клиент (Frontend) отправляет запрос (имя файла, тип контента) на эндпоинт **FastAPI бэкенда на Koyeb** (например, `/api/v1/generate-upload-url`). Запрос защищен **Firebase Authentication** (проверка JWT токена).
  - **Получение преподписанного URL:** FastAPI бэкенд генерирует преподписанный URL для **Cloudflare R2** и возвращает его клиенту вместе с необходимыми полями формы и `object_key`.
  - **Загрузка Медиа-файлов ("чанков"):** Клиент загружает файл напрямую в **Cloudflare R2**, используя полученный преподписанный URL и поля.
  - **Уведомление о Загрузке и Отправка Метаданных/Команд:** После успешной загрузки в R2, клиент отправляет метаданные "чанка" (включая `object_key` из R2) и другие команды через HTTPS-запросы к соответствующим эндпоинтам **FastAPI бэкенда на Koyeb** (например, функция `processChunkMetadata` или `triaChatHandler`). Все запросы защищены **Firebase Authentication**.
Обработка на Бэкенде (FastAPI приложение на Koyeb, основная логика в `backend/core/` и `backend/routers/`):
Прием и Валидация:
  - Эндпоинты FastAPI (например, `triaChatHandler` или новый эндпоинт для метаданных после загрузки) получают данные запроса, валидируют их структуру (Pydantic модели) и JWT токен пользователя.
Сохранение Сырых Данных и Метаданных: FastAPI эндпоинт сохраняет метаданные "чанка" (включая `object_key` или URL файла в Cloudflare R2) в базу данных **Neon.tech PostgreSQL** (например, таблица `audiovisual_gestural_chunks` или `raw_interactions`). Данные привязываются к `user_id`.
Передача Оркестратору Триа (модуль в `backend/core/tria_logic.py` или будущий Genkit flow): Обрабатывающий FastAPI эндпоинт инициирует логику Триа, передавая ей данные "чанка" и `user_id`.
Работа Ботов (модули в `backend/core/tria_bots/`, вызываемые из FastAPI сервисов или Genkit flows):
  - GestureBot: Анализирует `landmark_data` (извлеченные на клиенте и переданные как часть метаданных) и `video_chunk` (ссылка на файл в Cloudflare R2). Классифицирует жест, извлекает параметры, генерирует `gesture_embedding`.
  - AudioBot: Анализирует `audio_chunk` (ссылка на файл в Cloudflare R2) для контекста или синтеза ответного звука.
  ...
  - LearningBot (асинхронно, например, через фоновую задачу FastAPI или отдельный сервис, триггерящийся событием Pub/Sub или по расписанию): Получает "чанк", результат его обработки и обратную связь пользователя для обновления моделей и базы знаний. Эволюционный принцип сохраняется.
...
Отправка Ответа: Ответ отправляется на фронтенд как HTTP-response от вызванного FastAPI эндпоинта. Для будущих реалтайм обновлений могут рассматриваться WebSockets (R&D).
...
Сбор Данных: Пользовательские взаимодействия ("чанки" в Cloudflare R2, метаданные в Neon.tech PostgreSQL) и обратная связь от пользователя сохраняются с привязкой к `user_id`.
Работа LearningBot (FastAPI Background Task / Genkit Flow / Отдельный сервис):
Анализ Эффективности: LearningBot (реализованный как фоновая задача в FastAPI, отдельный сервис, или Genkit flow, возможно, запускаемый по расписанию или Pub/Sub) периодически анализирует накопленные данные. Оценивает, какие конфигурации/модели других ботов были наиболее успешны.
...
*   Каждый полезный "чанк" данных от пользователя (загруженный в R2 и обработанный FastAPI), который способствует обучению Триа (особенно с подтвержденной обратной связью), может генерировать пользователю "Инфокойны".
...
Триа (Tria): AI-ядро проекта. На этапе MVP – набор логик, реализованных в **FastAPI приложении на Koyeb (Python)**, использующих прямые вызовы LLM API. В перспективе – сеть специализированных, эволюционирующих ботов, оркестрируемых с помощью Genkit.
Комбинированный Чанк Взаимодействия (Interaction Chunk): Синхронизированный набор данных (аудио, видео, жесты, метаданные), фиксирующий одно взаимодействие пользователя с системой. Является основной "пищей" для обучения Триа. Загружается в **Cloudflare R2 (через presigned URLs)**, метаданные сохраняются в Neon.tech PostgreSQL.
...
3. Архитектура Потоков Данных и Взаимодействия (FastAPI на Koyeb MVP)
3.1. Сценарий: Взаимодействие Пользователя с Голограммой (Жест -> Звук/Визуализация) – Обновлено для FastAPI на Koyeb
Этот сценарий описывает основной цикл взаимодействия пользователя с системой в рамках MVP на FastAPI, развернутом на Koyeb.
...
Формирование "Чанка" (Interaction Chunk):
  - Клиентский JavaScript (`frontend/js/services/firebaseStorageService.js` теперь работает с R2) формирует объект с метаданными (например, тип данных, `timestamp`, `user_id` от Firebase Auth). Файл медиа готовится к загрузке.
Отправка на Бэкенд:
  - **Запрос преподписанного URL:** Клиент (Frontend) через `frontend/js/services/apiService.js` отправляет запрос (имя файла, тип контента) на эндпоинт **FastAPI бэкенда на Koyeb** (например, `/api/v1/generate-upload-url`). Запрос защищен **Firebase Authentication** (проверка JWT токена).
  - **Получение преподписанного URL:** FastAPI бэкенд генерирует преподписанный URL для **Cloudflare R2** и возвращает его клиенту вместе с необходимыми полями формы и `object_key`.
  - **Загрузка Медиа-файлов:** Клиент загружает файл напрямую в **Cloudflare R2**, используя полученный преподписанный URL и поля.
  - **Уведомление о Загрузке и Отправка Метаданных/Команд:** После успешной загрузки в R2, клиент отправляет метаданные "чанка" (включая `object_key` из R2) и другие команды через HTTPS-запросы к соответствующим эндпоинтам **FastAPI бэкенда на Koyeb** (например, эндпоинт для `processChunkMetadata` или `triaChatHandler`). Все запросы защищены **Firebase Authentication**.
Обработка на Сервере (Бэкенд / FastAPI приложение на Koyeb, логика в `backend/routers/`, `backend/core/`):
Прием и Выполнение Эндпоинтов FastAPI:
  - Эндпоинты FastAPI (например, в `backend/routers/chunks.py` или `backend/routers/chat_sessions.py`) обрабатывают входящие запросы.
Валидация и Аутентификация: Каждый защищенный эндпоинт FastAPI проверяет JWT токен пользователя через зависимости, настроенные в `backend/auth/security.py` (с использованием `firebase_admin.auth` на стороне сервера). Pydantic модели используются для валидации данных запроса.
Сохранение Данных: Эндпоинты FastAPI используют `asyncpg` (через модули в `backend/core/db/`) для сохранения/извлечения данных в/из **Neon.tech PostgreSQL**. Сохраняются метаданные чанков (ссылка/ключ на файл в R2, `user_id`, `timestamp` и др.), пользовательские жесты, состояния голограмм, история чата в соответствующие таблицы.
Логика Триа (внутри FastAPI для MVP):
  - Простая обработка: Для MVP, Триа-логика (например, в обработчиках маршрутов) может включать прямые вызовы LLM API (Google Gemini, Mistral) с использованием клиентов из `backend/core/services/llm_service.py` или аналогичных.
  - Модули Ботов (`backend/core/tria_bots/`): Концептуально, логика может быть структурирована с использованием модулей `ChatBot.py`, `ChunkProcessorBot.py`, `MemoryBot.py` (с RAG из Neon.tech), вызываемых внутри сервисов FastAPI.
  - LearningBot (асинхронно, Post-MVP): Может быть реализован как фоновая задача FastAPI (`BackgroundTasks`), отдельный сервис, или Genkit flow, триггерящийся, например, по таймеру или событию Pub/Sub, для обработки накопленных данных и обратной связи.
Формирование Ответа Триа: FastAPI эндпоинт формирует ответ (текст, команды для UI/голограммы).
Отправка Ответа на Фронтенд: Ответ возвращается как HTTP-response от FastAPI.
...
Непрерывный Сбор Данных: Все Interaction Chunks (метаданные в Neon.tech, файлы в Cloudflare R2) и обратная связь от пользователя сохраняются с привязкой к `user_id`.
Работа LearningBot (FastAPI Background Task / Genkit Flow / Отдельный сервис):
Анализ Эффективности: LearningBot (реализованный как фоновая задача FastAPI, отдельный сервис, или Genkit flow, возможно, запускаемый по расписанию или Pub/Sub) периодически анализирует данные.
...
Основные Концепции (пересмотрены для FastAPI на Koyeb MVP):
Голограмма (Hologram): Динамическая 3D аудиовизуализация. MVP использует Three.js/WebGL. WebGPU – R&D.
Триа (Tria): AI-ассистент. MVP – логика в **FastAPI приложении на Koyeb (Python)**, прямые вызовы LLM. Будущее – Genkit.
Комбинированный Аудио(Видео)-Жестовый Чанк (Interaction Chunk): Данные взаимодействия пользователя. Загружаются в **Cloudflare R2 (через presigned URLs)**, метаданные в Neon.tech PostgreSQL.
Firebase Ecosystem: Используется для Frontend Hosting и Firebase Authentication.
Neon.tech PostgreSQL + pgvector: Активная база данных для MVP.
Cloudflare R2: Основное хранилище файлов для MVP.
FastAPI on Koyeb: Основа бэкенд-инфраструктуры MVP.
Docker: Для контейнеризации FastAPI бэкенда.
Genkit: Целевой фреймворк для оркестрации AI-логики Триа (post-MVP).

Ключевые Директории и Файлы Проекта (Актуализировано для FastAPI на Koyeb):
`backend/`: Содержит Python-код для FastAPI приложения.
  `backend/app.py`: Точка входа FastAPI приложения, инициализация, подключение роутеров.
  `backend/routers/`: Модули FastAPI APIRouter для различных групп эндпоинтов.
  `backend/api/v1/endpoints/`: Дополнительные модули эндпоинтов, могут быть структурированы по версиям.
  `backend/core/`: Общая переиспользуемая логика, импортируемая обработчиками FastAPI:
    `backend/core/db/`: Модули для работы с Neon.tech PostgreSQL.
    `backend/models/`: Pydantic модели для валидации данных API и структуры таблиц БД (перемещено из `core/models`).
    `backend/core/services/`: Клиенты и сервисы для взаимодействия с внешними API (например, LLM).
    `backend/core/tria_bots/`: Логика отдельных ботов Триа.
  `backend/auth/`: Логика аутентификации для FastAPI, использующая Firebase Admin SDK.
  `backend/requirements.txt`: Список Python-зависимостей для FastAPI приложения.
  `backend/tests/`: Юнит-тесты и интеграционные тесты для FastAPI.
`frontend/`: Код фронтенда, развертываемый на Firebase Hosting.
  ...
  `frontend/js/`: Корневая директория для JavaScript ES6 модулей.
    ...
    `frontend/js/services/firebaseStorageService.js` (теперь `r2StorageService.js` или аналогичный, отвечает за загрузку на R2): Управляет загрузкой файлов в Cloudflare R2 через presigned URLs.
...
Корневые файлы конфигурации Firebase:
  ...
  `firebase.json`: Конфигурирует развертывание Firebase Hosting и настройки Firebase Local Emulator Suite. (Раздел `functions` удален или не используется для основного бэкенда).
Прочие файлы:
...
Сеть Ботов Триа (концептуально, реализуется в `backend/core/tria_bots/` и вызывается из FastAPI; в будущем оркестрируется Genkit):
AudioBot, GestureBot/VideoBot, MemoryBot, LearningBot, CoordinationService (Оркестратор – в будущем Genkit flow).

5. Блок-схема Верхнего Уровня (Обновлено для FastAPI на Koyeb MVP)
*(Примечание: Mermaid-диаграмма ниже была обновлена для отражения архитектуры FastAPI на Koyeb, но может требовать дальнейшей детализации для сложных взаимодействий.)*

**Пояснения к обновленной блок-схеме (FastAPI на Koyeb MVP):**
...
3.  Для загрузки медиа-"чанков" (аудио, видео), Frontend:
    a.  Запрашивает **Presigned URL** у **FastAPI бэкенда (Koyeb)**.
    b.  Загружает файл напрямую в **Cloudflare R2** используя полученный presigned URL.
    c.  Уведомляет **FastAPI бэкенд (Koyeb)** об успешной загрузке, передавая метаданные и `object_key`.
4.  Для выполнения другой бизнес-логики, отправки команд или текстовых данных, Frontend делает HTTPS-запросы к **FastAPI бэкенду (Koyeb)**, передавая JWT токен для авторизации.
5.  **FastAPI бэкенд (Koyeb)** является ядром бэкенда:
    *   Проверяют JWT токен пользователя с помощью Firebase Admin SDK (через зависимости в `backend/auth/security.py`).
    ...
    *   В будущем, эта логика может быть интегрирована или оркестрирована с помощью **Genkit** flows.
6.  FastAPI бэкенд возвращают HTTP-ответы (например, JSON) на Frontend.
7.  Frontend соответствующим образом обновляет UI, визуализацию голограммы, воспроизводит аудио и т.д.
8.  **HoloGraph (Блокчейн)** и связанные с ним P2P-механизмы остаются долгосрочным R&D направлением и не являются частью основного потока MVP.
...
graph LR
    ...
    subgraph Koyeb Hosting
        FASTAPI_BACKEND[FastAPI Бэкенд (Docker)]
    end

    subgraph Backend Logic & Data (внутри FastAPI или вызывается им)
    ...
    B -- 1. Запрос Presigned URL (HTTPS с JWT) --> FASTAPI_BACKEND
    FASTAPI_BACKEND -- 2. Возврат Presigned URL --> B
    B -- 3. Загрузка чанков (медиа) по Presigned URL --> R2_STORAGE
    B -- 4. Уведомление о загрузке, метаданные (HTTPS с JWT) --> FASTAPI_BACKEND

    B -- Другие API Запросы (HTTPS с JWT) --> FASTAPI_BACKEND

    FASTAPI_BACKEND -- Проверка токена с Firebase Admin --> FB_AUTH
    FASTAPI_BACKEND -- Данные/Команды --> TRIA_LOGIC
    TRIA_LOGIC -- CRUD, RAG --> DB
    TRIA_LOGIC -- Запросы --> LLM_API
    LLM_API -- Ответы --> TRIA_LOGIC
    DB -- Данные --> TRIA_LOGIC
    TRIA_LOGIC -- Результат --> FASTAPI_BACKEND
    FASTAPI_BACKEND -- HTTP Response --> B
