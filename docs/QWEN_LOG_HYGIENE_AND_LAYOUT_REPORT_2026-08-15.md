...
### 5. 🎯 Релиз v0.20.602: Устранение P0 P2P Presence спама, восстановление канонической красно-фиолетовой сетки и автозума

2026-08-16 12:30 MSK — **P0 P2P Presence TypeError (`js/core/init.js`):** Вызовы `state.holoEngine?.engine?.addPresenceMarker(...)` исправлены на `state.holoEngine?.addPresenceMarker(...)` с предварительной проверкой `typeof === 'function'`.

2026-08-16 12:30 MSK — **Страховка в тактомере (`js/core/TriaPulse.js`):** В метод `_beat` добавлен try-catch error boundary с троттлингом (ошибки эмиттера в RAF-цикле логируются не чаще раза в 30 секунд), что гарантирует защиту от лавинных 60 FPS спам-циклов при любых сбоях подписчиков.

2026-08-16 12:30 MSK — **Восстановление канонической сетки (`js/engine/GridWireframe.js`):** Устранены тускло-серые линии с шагом 32. Полностью восстановлена историческая каноническая сетка голограммы:
- **Левая сетка ($X \in [0, -128]$):** Фиолетовый цвет `#BF00FF` (`[0.749, 0.0, 1.0]`) с шагом 16 единиц.
- **Правая сетка ($X \in [0, +128]$):** Красный цвет `#FF0000` (`[1.0, 0.0, 0.0]`) с шагом 16 единиц.
- **Оси координат и сфера:** Красная ось $X+$, фиолетовая $X-$, зелёная $Y+$, белая $Z+$ и синяя сферическая точка на пересечении осей.

2026-08-16 12:30 MSK — **Калибровка зума ортокамеры (`js/ui/layoutManager.js`):** Зум `holoCam.setZoom` синхронизирован с соотношением `usableHeight / containerHeight` и `usableWidth / (containerHeight * aspect)`. Голограмма и сетка целиком помещаются на экране как в свернутом, так и в развернутом виде панелей, без обрезания границ и наезда на камеру.
