Системная Инструкция: Проект "Голографические Медиа" (holograms.media) – Версия 32.1 (Koyeb Update)
Дата: 10 Июля 2024 г. (12:00 PM UTC, среда)

A. Общая Шапка Документа
(Версия и дата обновлены выше)

B. Финальный Вектор Развития (Актуализировано для Koyeb/R2/Neon MVP, с сохранением R&D из v30.0++)
...
- Разработка интуитивных жестовых интерфейсов (включая поддержку русского жестового языка – РЖЯ – R&D, базовая интеграция MediaPipe Hands для MVP).
- Создание самообучающейся сети ботов "Триа" (концепции AZR, "жидкий код" – долгосрочно, MVP фокусируется на простой логике Триа **на FastAPI (размещенном на Koyeb)** с перспективой использования Genkit).
- Внедрение децентрализованной экономической системы "HoloGraph" (долгосрочное R&D).
- **Обновленная архитектура MVP**:
...
E. ПРЕАМБУЛА: МАНИФЕСТ ПРОЕКТА "ГОЛОГРАФИЧЕСКИЕ МЕДИА" (Адаптировано к Koyeb/R2/Neon MVP)
Мы стоим на пороге новой эры коммуникации... Для достижения MVP мы используем комбинацию экономичных и мощных сервисов: **Koyeb для бэкенда (Hobby Plan, Frankfurt), Cloudflare R2 для хранения файлов, Neon.tech для базы данных, и Firebase для хостинга фронтенда и аутентификации, стремясь к использованию бесплатных тиров без необходимости предоставления банковской карты.**

Центральной фигурой этой экосистемы является AI-ассистент "Триа". На этапе MVP Триа представляет собой **FastAPI приложение на Koyeb**, способное обрабатывать пользовательские данные ("комбинированные аудиовизуально-жестовые чанки") и предоставлять базовые ответы. В долгосрочной перспективе Триа будет эволюционировать...
...
1.1. Общая Концепция (Всеобъемлющая – Актуализировано для Koyeb/R2/Neon MVP)
Проект "Голографические Медиа" (holograms.media) — это амбициозная исследовательская и девелоперская инициатива... Для MVP (Minimum Viable Product) платформа реализуется с использованием следующего стека, ориентированного на доступность и бесплатные тиры:
...
-   **AI "Триа" (MVP)**: Логика реализована на **FastAPI эндпоинтах** и использует API ключи для доступа к LLM (например, Mistral).

Ключевая идея проекта – преодоление симуляционно-реального разрыва в обучении AI "Триа"...

Технологическая основа проекта (MVP и долгосрочные цели, с сохранением деталей из v30.0++):
...
**Бэкенд-технологии (MVP)**: **FastAPI на Koyeb (Python 3.11+, Hobby Plan, Frankfurt).**
**База данных (MVP)**: **Neon.tech PostgreSQL + pgvector.** Подключение через `asyncpg` или `psycopg2` из FastAPI.
...
**Развертывание (MVP)**: Frontend на Firebase Hosting, Backend на **Koyeb (Hobby Plan, Frankfurt)**, База Данных (Neon.tech), Хранилище (**Cloudflare R2**), Аутентификация (Firebase Authentication).
...

1.2. Ключевые Направления Разработки (Стратегические и Тактические – Актуализировано для Koyeb/R2/Neon MVP, с сохранением R&D деталей из v30.0++)
...
B. БЭКЕНД: РЕАЛИЗАЦИЯ MVP ЛОГИКИ НА **KOYEB (FASTAPI)** (Высший приоритет)

Основной фокус: Реализация всей бэкенд-логики для MVP в виде **FastAPI приложения на Python, развернутого на Koyeb (Hobby Plan, Frankfurt) через интеграцию с GitHub (возможно, с использованием Dockerfile для сборки)**, как детализировано в `ULTIMATE_ROAD_TO_MVP_JUNE_9.md`.
Ключевые задачи:
...
Глубокое и Актуальное Документирование: ... Особое внимание уделяется документированию конфигураций **FastAPI, Koyeb, R2, Neon.tech** и логики бэкенд-сервисов.
Безопасность и Приватность (**Koyeb/R2/Neon-ориентированный подход**): ... Переменные окружения на **Koyeb** для управления API-ключами...
Среда Разработки и Развертывания (**Новый Стек**):
...
1.6. Долгосрочные Видение и Цели (Манифест Будущего – Адаптировано к Koyeb/R2/Neon MVP как основе)
MVP, построенный на **Koyeb, Cloudflare R2, Neon.tech и Firebase**, является первым шагом к реализации нашего долгосрочного видения.
...
AI-Ассистент "Триа" – Эволюционирующий Партнер: Развитие Триа от MVP-реализации на **FastAPI (на Koyeb)** к полноценной самообучающейся сети ботов...

II. Архитектура и Текущее Состояние
2.1. Инфраструктура и Развертывание (Новый Стек MVP: Koyeb, Cloudflare R2, Neon.tech, Firebase)
...
-   **AI Orchestration (MVP)**: Логика AI "Триа" реализована в FastAPI эндпоинтах на Koyeb...
...
2.4. AI "Триа" (MVP и Перспективы, адаптировано)
Бэкенд-логика Триа для MVP: Реализована в **FastAPI эндпоинтах на Koyeb**...
Взаимодействие с Данными: Триа (через **FastAPI на Koyeb**) читает и записывает данные в Neon.tech PostgreSQL и **Cloudflare R2**.

2.6. Целевая Архитектура (Высокоуровневая. Перспектива нового стека)
Backend: **FastAPI приложение на Koyeb (MVP, Hobby Plan, Frankfurt)**. В дальнейшем, для масштабирования, могут рассматриваться платные планы Koyeb или другие PaaS/IaaS решения.
Развертывание и Инфраструктура: **Koyeb, Cloudflare R2, Neon.tech, Firebase Hosting/Auth (MVP)**.
...
III. Ключевые Проблемы и Задачи (MVP Focus на Koyeb/R2/Neon)
3.1. Общая Стратегия Решения Проблем
... используя **новый стек: Koyeb для бэкенда, Cloudflare R2 для хранения, Neon.tech для БД и Firebase для фронтенда/аутентификации**.

3.2. Основные Технические Вызовы для MVP (Новый Стек):
...
Реализация Бэкенда на **Koyeb (FastAPI)**:
Задача: Создание и тестирование **FastAPI приложения** для всей серверной логики MVP, развернутого на Koyeb.
...
Задача: Реализация MVP в рамках бесплатных или очень экономичных квот **Koyeb, Cloudflare R2, Neon.tech, Firebase Hosting/Auth**.
Безопасность в **Новом Стеке**:
...
AI-Логика Триа для MVP (на FastAPI, хостинг на Koyeb):
...
... Развитие Триа строится на основе **FastAPI бэкенда на Koyeb для MVP**...

4.2. Фаза 1: Основы и Стабильность (Текущий Этап – MVP на FastAPI/Koyeb)
...
Базовая Логика Триа в **FastAPI (на Koyeb)** (MVP):
...
4.5. Риски и Меры (Новый Стек Focus)
...
5.7. Приоритеты для AI (MVP Focus на Koyeb/R2/Neon)
1.  **Реализация MVP на Koyeb/R2/Neon/Firebase**: Помощь в разработке и отладке кода для фронтенда (Firebase Hosting) и бэкенда (**FastAPI на Python, деплой на Koyeb**).
...
IX. Обратная Связь и Планы Тестирования (Обновлено для Koyeb MVP)
9.1. Общие Принципы Тестирования... MVP фокусируется на достижении стабильной MVP-функциональности **на стеке Koyeb, Cloudflare R2, Neon.tech и Firebase**.

9.2. План Внутреннего Тестирования (Новый Стек)
Цель: Проверить работоспособность ключевых функций MVP, развернутых на **Koyeb и Firebase Hosting**...
...
Мера: Осознание этих ограничений, фокус на функциональном тестировании для MVP. Использование платных планов Koyeb или других платформ для более серьезного тестирования производительности.

X. Долгосрочные Цели и Видение (Актуализировано с учетом Koyeb MVP)
10.1. Общее Видение Проекта
... MVP, построенный на **Koyeb, Cloudflare R2, Neon.tech и Firebase**, является первым шагом...
