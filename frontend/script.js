...
async function loadInitialFilesAndSetupEditor() {
    await Promise.all([
        ...
        if (fileContentTextAreaElement && fileContents['/static/script.js']) {
             ...
             fileContentTextAreaElement.dataset.currentFile = '/static/script.js';
              if (fileListElement) {
                   fileListElement.querySelectorAll('li').forEach(item => {
      ...
      .then(result => {
        ...
        axios.post('/generate', { prompt, model })
          .then(generateResponse => {
            const backgroundColor = generateResponse.data.backgroundColor;
            const generatedCode = generateResponse.data.generatedCode;

            if (generatedCode) {
              try {
                const executeCode = new Function('scene', 'mainSequencerGroup', 'THREE', generatedCode);
                executeCode(scene, mainSequencerGroup, THREE);
              } catch (e) {
                alert(`Ошибка выполнения сгенерированного кода:\n${e.message}\n\nПромт: ${prompt}`);
              }
            }

            const sceneStateObject = JSON.parse(JSON.stringify(scene.toJSON()));
            ...
            return axios.post('/branches', {
              ...
              files: {
                  ...fileContents,
                  'generated_code.js': generatedCode || ''
              },
  ...
  window.addEventListener('resize', () => {
        ...
        const gestureAreaElement = document.getElementById('gesture-area');
        const handsAreCurrentlyVisible = gestureAreaElement ? (gestureAreaElement.style.height !== '4px');
  ...
  function initializeMediaPipeHands() {
    ...
    console.log("MediaPipe Hands initialized with settings");
    ...
    async function startVideoStream(videoElement, handsInstance) {
      ...
      if (!iriunDeviceId && videoDevices.length > 0) {
          iriunDeviceId = videoDevices[0].deviceId;
