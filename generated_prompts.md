...
- **Атомарное действие:** Модифицировать файл `backend/core/db/schema.sql`, добавив определения SQL для таблиц `tria_learning_log` и `user_chat_sessions`.
    - `tria_learning_log`: Должна включать поля для ID взаимодействия, ID пользователя, ID сессии, временной метки, входных данных пользователя, ответа Tria, и возможно, метаданных LLM.
    - `user_chat_sessions`: Должна включать поля для ID сессии, ID пользователя, временной метки начала, и возможно, временной метки последнего сообщения.
