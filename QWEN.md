...
**Holograms Media** — это мультимодальная XR-платформа для визуализации и взаимодействия со звуком в реальном времени. Проект использует **BasilaQ-256** — систему аудио-анализа через CWT (Continuous Wavelet Transform), преобразующую звук в 3D-голограмму с 128 частотными полосами, 128 уровнями громкости и 360° панорамой. BasilaQ-256: 2 уха × 128 бинов = 256 колонн тора вокруг пользователя.
...
- **HoloEngine** — собственный WebGPU 3D-движок для визуализации BasilaQ-256
...
|-----------|-----------|------------|
| **HoloEngine** | WebGPU (чистый WGSL) | 3D рендеринг голограммы BasilaQ-256 |
...
| **X** | Панорама | 128 позиций (1.40625° = 1 ячейка) — константа визуальной сетки |

**BasilaQ-256**: 2 уха × 128 бинов = 256 колонн тора вокруг пользователя. Угловой шаг сетки остаётся 1.40625° = 180°/128 (неизменно).
