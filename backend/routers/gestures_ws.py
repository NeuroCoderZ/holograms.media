...
async def websocket_endpoint(
    ...
    logger.info(f"WebSocket connection established for user {'unknown (auth pending fix)'}")
    ...
    try:
        while True:
            ...
            if not intent_val:
                ...
                logger.warning(f"Received WebSocket message without intent from user {'unknown'}")
            ...
            logger.info(f"Received intent_data from user {'unknown'}: {data}")
            ...
            user_identifier = "anonymous_websocket_user" # Fallback, если user почему-то None
        ...
        logger.info(f"WebSocket connection closed for user {'unknown'}")
    except Exception as e:
        # logger.error(f"Error in WebSocket for user {user.firebase_uid if user else 'unknown'}: {e}", exc_info=True)
        logger.error(f"Error in WebSocket for user {'unknown'}: {e}", exc_info=True)
