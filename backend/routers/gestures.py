...
async def create_new_user_gesture(
    ...
    try:
        ...
        # UserInDB model now has firebase_uid, email, role, etc. but not username directly.
        # Logging current_user.firebase_uid instead of current_user.username
        ...
        created_gesture = await crud_operations.create_user_gesture(
            conn=db_conn, user_id=current_user.firebase_uid, gesture_in=gesture_in # Use firebase_uid
        )
...
async def get_specific_user_gesture(
    ...
    gesture = await crud_operations.get_user_gesture_by_id(
        conn=db_conn, gesture_id=gesture_id, user_id=current_user.firebase_uid # Use firebase_uid
    )
    if not gesture:
...
async def update_existing_user_gesture(
    ...
    updated_gesture = await crud_operations.update_user_gesture(
        conn=db_conn, gesture_id=gesture_id, user_id=current_user.firebase_uid, gesture_update_data=update_data # Use firebase_uid
    )
