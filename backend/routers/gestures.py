...
from backend.services.gesture_service import GestureService # <-- НОВЫЙ ИМПОРТ
...
router = APIRouter(
    ...
    # Тег был "User Gestures", но в app.py для legacy_user_me_gestures_router указан "Current User Gestures (Legacy)"
    # Оставляем как "Current User Gestures (Legacy)" для консистентности с app.py
    tags=["Current User Gestures (Legacy)"],
)

# Модель GestureUpdate используется для PUT запроса.
# Она должна быть определена здесь или в gesture_models.py.
# В gesture_models.py ее нет. Оставляем ее здесь, как было.
class GestureUpdate(gesture_models.UserGestureDefinitionBase):
    ...
    # Используем exclude_none=True в dict() вместо этого конструктора, если возможно,
    # но для соответствия оригиналу оставим конструктор.
...
async def create_new_user_gesture(
    ...
    try:
        ...
        # Передаем user_id из current_user, а не из gesture_in, если оно там есть
        ...
        if not created_gesture:
            # Сервис может вернуть None при конфликте (например, дубликат имени)
        ...
        raise
    except Exception as e: # Ловим другие ошибки, которые могли возникнуть в сервисе/репозитории
...
async def update_existing_user_gesture(
    ...
    if not updated_gesture:
        # Сервис может вернуть None при ошибке (например, не найдено или конфликт имени)
