...
-   **`core/`:** Houses shared business logic, services, and utilities.
    ...
    *   **`auth/` (at `backend/auth/`):** Contains authentication logic, including security dependencies for FastAPI using JWT tokens.
...
-   **`.env.example`:** Provides a template for environment variables needed for local development and production (e.g., database connection strings, API keys).
...
2.  **Environment Variables:**
    *   Create a `.env` file by copying `.env.example`:
        ...
        cp .env.example .env
        ```
    *   Populate the `.env` file with your actual credentials for Astra DB (DataStax) and any other required API keys.
...
-   Any other API keys (e.g., `MISTRAL_API_KEY`, `GOOGLE_API_KEY`).
...
*   **Security:** Secure API endpoints using JWT Authentication (via `backend.auth.security`). Review CORS configurations.
