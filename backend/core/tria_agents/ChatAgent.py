# backend/core/tria_agents/ChatAgent.py
...
class ChatAgent:
    ...
    async def get_response(self, user_input: str, user_id: str) -> str:
        """
        Generates a conversational response from the Tria agent based on user input.
        ...
        logger.info(f"Sending prompt to LLM for user {user_id}: {user_input[:100]}...")
        try:
            ...
            logger.info(f"Received response from LLM for user {user_id}: {llm_response[:100]}...")
            return llm_response
