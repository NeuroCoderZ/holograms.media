...
class InternalMessage(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=current_time_utc,
        description="Timestamp when the message was created.",
    )
    source_service: str = Field(
        ...,
        description="Identifier of the service that originated this message (e.g., 'GestureAgent', 'FrontendClient', 'NetHoloGlyphService').",
    )
    target_service: Optional[str] = Field(
        default=None,
        description="Identifier of the intended recipient service, if specific (e.g., 'CoordinationService', 'MemoryAgent'). Can be None for broadcast-like events.",
    )
    event_type: str = Field(
        ...,
        description="Type of the event or message (e.g., 'new_gesture_chunk_received', 'tria_state_update_request', 'holographic_symbol_render_command').",
    )
    payload: Dict[str, Any] = Field(
        default_factory=dict,
        description="The actual data payload of the message. Structure depends on event_type.",
    )
    correlation_id: Optional[str] = Field(
        default=None,
        description="Optional ID to correlate related messages, e.g., a request and its subsequent response or a series of events in a flow.",
    )

    # Fields added from backend/models/internal_bus_models.py
    user_id: Optional[str] = Field(
        None,
        description="Unique user ID of the user associated with this event, if applicable.",
    )
    session_id: Optional[str] = Field(
        None, description="Session ID associated with this event, if applicable."
    )
    priority: Optional[int] = Field(
        default=0, description="Message priority, if applicable."
    )

    class Config:
        from_attributes = True  # Kept from core version
        extra = "forbid"  # Added from models version
        json_schema_extra = {  # Kept from core version
            "examples": [
                {
