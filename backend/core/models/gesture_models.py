...
class GesturalPrimitive(BaseModel):
    ...
    type: str = Field(
        ...,
        description="e.g., 'pinch_start', 'swipe_left', 'hold', 'point_at_object_X'",
    )
    timestamp: float = Field(
        ...,
        description="Timestamp of the primitive event relative to sequence start or absolute.",
    )
    hand: Optional[str] = Field(
        None, description="'left', 'right', 'both'"
    )  # Make optional as some systems might not specify
    confidence: Optional[float] = Field(None, ge=0.0, le=1.0)
    spatial_data: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Detailed coordinates (e.g. x,y,z of fingertip), orientation, proximity, target object ID",
    )
...
class InterpretedGestureSequenceBase(BaseModel):
    duration_ms: Optional[float] = (
        None  # Total duration of the interpreted gesture sequence
    )
    primitives: List[GesturalPrimitive] = Field(default_factory=list)
    semantic_hypotheses: List[Dict[str, Any]] = Field(
        default_factory=list,
        description="List of semantic hypotheses with confidence, e.g., [{'intent': 'create_cube', 'params': {'size': 0.5}, 'confidence': 0.8}]",
    )
    context_snapshot: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Relevant state of the holographic environment at the time of gesture",
    )
    # Additional fields from DRSB
    source_modality: Optional[str] = Field(
        default="hand_tracking",
        description="e.g., 'hand_tracking', 'full_body_tracking', 'eye_gaze'",
    )
    raw_data_references: Optional[List[str]] = Field(
        default_factory=list,
        description="Links to original chunk IDs or time-series data if applicable",
    )
...
class UserGestureDefinitionBase(BaseModel):
    ...
    gesture_definition: Dict[str, Any] = Field(
        ..., description="User-defined parameters or sequence for the gesture"
    )
    gesture_data_ref: Optional[str] = Field(
        None, description="Optional link to an example audiovisual_gestural_chunks.id"
    )
...
class GestureUpdate(UserGestureDefinitionBase):
    ...
    gesture_name: Optional[str] = Field(
        None, min_length=1, max_length=100
    )  # Re-declare to make Optional
    gesture_definition: Optional[Dict[str, Any]] = Field(
        None
    )  # Re-declare to make Optional
...
from .base_models import (
    BaseUUIDModel,
)  # Assuming BaseUUIDModel provides id, created_at, updated_at
...
class CoreInterpretedGestureModel(
    BaseModel
):  # Corresponds to InterpretedGestureSequence in DRSB
    semantic_meaning: str  # e.g., "select_object", "navigate_forward"
    confidence: float
    target_object_id: Optional[UUID] = Field(
        default=None, description="ID of the object this gesture targets, if any."
    )
    parameters_json: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Parameters associated with the gesture command, e.g., {'speed': 'fast'}.",
    )
...
class CoreGestureModel(BaseUUIDModel):  # Inherits id, created_at, updated_at
    user_id: UUID = Field(
        ..., description="ID of the user who performed or owns this gesture."
    )
    gesture_name: Optional[str] = Field(
        default=None, description="User-defined name for this gesture, if saved."
    )

    recognized_gesture_type: Optional[str] = Field(
        default=None,
        description="High-level recognized gesture type (e.g., 'swipe_left', 'pinch_zoom').",
    )
    gesture_primitives_json: Optional[List[CoreGesturePrimitiveModel]] = Field(
        default=None, description="Sequence of recognized gesture primitives."
    )  # Updated type hint

    landmark_data_3d_json: Optional[List[Dict[str, float]]] = Field(
        default=None,
        description="List of 3D landmark data points (e.g., [{'x':0.1, 'y':0.2, 'z':0.3}, ...]). Each dict could represent a landmark.",
    )

    source_modality: Optional[str] = Field(
        default=None,
        description="Source of the gesture data (e.g., 'MediaPipe_Web', 'WebXR_HandTracking', 'LeapMotion').",
    )

    tria_interpretation: Optional[CoreInterpretedGestureModel] = Field(
        default=None,
        description="Tria's interpretation of the gesture's meaning and intent.",
    )  # Updated type hint

    context_hologram_id: Optional[UUID] = Field(
        default=None,
        description="ID of the hologram active or targeted when this gesture was made.",
    )
    custom_metadata_json: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Other custom metadata."
    )
