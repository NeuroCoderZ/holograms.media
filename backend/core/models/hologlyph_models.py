...
class HolographicSymbolModel(BaseUUIDModel):  # Kept from original core
    symbol_id_str: str = Field(
        ...,
        description="A human-readable or context-specific ID for the symbol, if different from the UUID 'id'.",
    )
    type: str = Field(
        ...,
        description="Type of the holographic symbol (e.g., 'cube', 'sphere', 'text_label', 'custom_model_uri').",
    )
    ...
    scale: Vector3Model = Field(
        default_factory=lambda: Vector3Model(x=1.0, y=1.0, z=1.0)
    )
    material_properties_json: Optional[Dict[str, Any]] = Field(
        default=None,
        description="JSON string or dict for material properties like color, texture URI, shininess etc.",
    )
    custom_data_json: Optional[Dict[str, Any]] = Field(
        default=None, description="Any other application-specific data for this symbol."
    )
    code_language: Optional[str] = Field(
        default=None,
        description="Language of associated code, if this symbol represents code ('liquid code').",
    )
    embedding_model_version: Optional[str] = Field(
        default=None, description="Version of the embedding model used, if applicable."
    )
...
class ThreeDEmojiModel(BaseUUIDModel):
    ...
    emoji_id_str: str = Field(
        ...,
        description="Unique ID for this instance of the emoji (maps to 'emoji_id' from backend/models version).",
    )
    type: str = Field(
        ..., description="Type of emoji (e.g., 'smile', 'thumbs_up', 'heart')."
    )  # From backend/models

    position: Vector3Model = Field(default_factory=Vector3Model)  # Consistent with core
    orientation: QuaternionModel = Field(
        default_factory=QuaternionModel
    )  # Consistent with core

    color_hex: Optional[str] = Field(
        None, description="Dominant color in hex (e.g., '#FFD700')."
    )  # From backend/models
    ...
    scale: float = Field(
        1.0, description="Uniform scale factor."
    )  # From backend/models

    animation_type: Optional[str] = Field(
        None, description="E.g., 'NONE', 'PULSE', 'SPIN_Y', 'BOUNCE'."
    )  # From backend/models
    animation_speed: Optional[float] = Field(
        default=1.0
    )  # From core, also in backend/models
...
class AudioVisualizationStateModel(BaseUUIDModel):  # Kept from original core
    stream_id: str = Field(
        ..., description="Identifier for the audio stream being visualized."
    )
    frequency_bands_json: Optional[List[float]] = Field(
        default=None, description="Data representing audio frequency bands."
    )
...
class TriaStateUpdateModel(BaseUUIDModel):  # Kept from original core
    state_key: str = Field(
        ...,
        description="Key identifying the part of Tria's state being updated (e.g., 'current_mood', 'active_task_id').",
    )
    state_value_json: Dict[str, Any] = Field(
        ..., description="The value of the state, can be complex."
    )
    bot_id: Optional[str] = Field(
        default=None,
        description="ID of the agent whose state is being updated, if applicable.",
    )
    pass
