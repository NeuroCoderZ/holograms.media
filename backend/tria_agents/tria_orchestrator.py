...
class TriaOrchestrator:
    ...
    async def _get_subagent_context(self, prompt: str, user_id: str, history: List[Dict] = None) -> str:
        """
        ...
        tasks = [tria_rag.get_relevant_context(kw, limit=3, user_id=user_id) for kw in keywords[:3]]
    ...
    async def stream_user_prompt(
        ...
        try:
            ...
            history_ctx = "\n".join([f"{m['role']}: {m['content']}" for m in (history or [])[-10:]])
