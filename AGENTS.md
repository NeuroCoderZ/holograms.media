...
## 🌐 ГРАФОВЫЙ АНАЛИЗ КОДОВОЙ БАЗЫ: CODEGRAPH И GRAPHIFY (ЭКОНОМИЯ ТОКЕНОВ) 🌐
2026-08-16 12:03 MSK — **СТРОГО ОБЯЗАТЕЛЬНО** для навигации, анализа архитектуры и поиска зависимостей по кодовой базе Holograms.media использовать связку двух взаимодополняющих графовых инструментов: **`codegraph`** и **`graphify`** вместо прямого вычитывания десятков файлов:
- **`codegraph` (`~/.npm-global/bin/codegraph`)**: Локальный AST/call-graph на базе SQLite.
  - Использовать для быстрого поиска связей и радиуса изменений: `codegraph impact "<symbol>"`, `codegraph callers "<symbol>"`, `codegraph callees "<symbol>"`, `codegraph query "<pattern>"`.
  - Синхронизация индекса: `codegraph sync .`.
- **`graphify` (`/home/neurocoderz/venvs/graphify/bin/graphify`)**: Семантический граф знаний и кластеризация кодовой базы.
  - Использовать для макро-анализа архитектуры и связности подсистем: `graphify extract --code-only` и `graphify cluster-only`.
- **Запрет на слепое чтение файлов:** Не тратить контекст и токены на чтение файлов целиком, пока через графы не определён точный узел и затронутые зависимости!
...
## Место в экосистеме NeuroCoderZ
Я — один из 10 подпроектов внутри корневого NeuroCoderZ. Все подпроекты — равноправные папки внутри общей структуры.

### Все 10 подпроектов NeuroCoderZ:
| # | Проект | Описание |
|---|--------|----------|
| 1 | **AgentMind** | Центральный RAG/RLM-контур + сенсорная подсистема, координатор (включая neurogrid_sensor) |
| 2 | **SCall** | Защищённый мессенджер, Android + YCF |
| 3 | **SNT_GUARD** | AI-видеонаблюдение, Hikvision + YOLO |
| 4 | **OlgaHerz** | Персональный AI для коуча Ольги |
| 5 | **NeuroOS** | Биохакинг и health dashboard |
| 6 | **holograms.media** | Spatial AI, VR, hand-tracking (ЭТОТ ПРОЕКТ) |
| 7 | **neuroescrow** | Smart contracts, Telegram Mini App |
| 8 | **gossecret** | Гос-почта, анализ документов |
| 9 | **TokenHunter** | Автоматический поиск бесплатных LLM API |
| 10 | **CoderzVoice** | Голосовой AI-ассистент / TTS-STT |
