...
export class HoloEngine {
    ...
    _setupCameras() {
        // Ортографическая камера (базовый вид)
        // Видим область: X: -140..140, Y: -10..138, Z: 0..160
        this.orthoProjection = this._ortho(-140, 140, -10, 138, 0.1, 300);

        // Перспективная камера (XR режим)
        this.perspectiveProjection = this._perspective(Math.PI / 4, 1.6, 0.1, 300);

        // Позиция камеры: смотрим на центр голограммы (0, 64, 64)
        // Камера на (0, 64, 160) — на расстоянии 160 единиц
        this.viewMatrix = this._lookAt([0, 64, 160], [0, 64, 64], [0, 1, 0]);
    }
