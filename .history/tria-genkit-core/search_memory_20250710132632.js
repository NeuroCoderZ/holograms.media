...
const GEMINI_MODEL = 'gemini-embedding-001'; // Updated to correct model name
...
function buildAdvancedFTS5Query(query) {
  ...
  // Создаем расширенный запрос с синонимами
  ...
  // Создаем гибкий запрос: точная фраза ИЛИ все слова ИЛИ синонимы
...
async function main() {
  try {
    ...
    console.error('\x1b[31mОтсутствует зависимость. Установите: npm install @google/generative-ai\x1b[0m');
    process.exit(1);
  }
  ...
  switch (mode) {
    ...
    default:
      if (!searchQuery) {
        console.error('\x1b[31mОшибка: Укажите поисковый запрос или режим работы.\x1b[0m');
        console.error('Используйте --help для просмотра справки.');
        process.exit(1);
      }
...
main().catch(error => {
  console.error('\x1b[31mКритическая ошибка:\x1b[0m', error.message);
  process.exit(1);
});
    await analyzeTopics();
  } else {
