...
| 3 | **Портировать слои на граф HoloEngine** — разблокирует удаление `THREE.Scene` | `js/engine/*`, `js/core/init.js` | ✅ **DONE** (3a: PresenceLayer, 3b: SmartHologram удалён, 3c: Three.js renderer из rendering.js убран) |
| 4 | **Удалить `THREE.Scene`/`OrthographicCamera`** из sceneSetup — перенос сцены завершён | `js/3d/sceneSetup.js` | ⬜ **БЛОКЕР=Шаг 6 (XR)**: `state.scene` нужен cameraManager (null-гард XR, `state.xrCamera && state.scene`) и init.js TorusVOM. Полное удаление сломает рабочий XR-режим. Не делать до Шага 6. |
| 5 | **Picking нативный** — заменить `THREE.Raycaster` на луч в HoloEngine (по pan/depth из dynamic-буфера) | `js/engine/Engine.js`, `js/core/threeImports.js` | ✅ **DONE 10.08** (`9a02435f`): `Engine.pickFromNDC` (инверсия proj×view; Raycaster был мёртв — никогда не инстанцировался) + **контур жестов замкнут**: `gesture:modify` → HoloEngine (orbit/zoom/hue/freq), раньше эмитил в пустоту |
...
| 7 | **Мультиплеер** — контракт состояния WebRTC/WS (`hologlyph-data`) над портом слоёв | `js/services/netHoloGlyphClient.js` | 🟡 **контур замкнут 10.08** (`cae4381d`+): найдено, что канал работал вхолостую — `eventBus.emit('netQuantum')` имел **0 подписчиков**, `gesture_frame` от пиров молча терялся (приёмник ждал только `intent-delta/soma_delta` из другого транспорта), а `sendQuantum` не штамповал отправителя → все пиры схлопывались в один маркер. Добавлены: подписчик `netQuantum` → `addPresenceMarker` (init.js), `peerId` в `sendQuantum`, тест `netQuantumContract.test.mjs`. Осталось: реальный e2e между двумя браузерами, WS-fallback-путь |
| 8 | **Вычитка Three-импортов** по модулю | все `js/3d/*`, `js/SmartHologram.js` | 🟡 **начата 10.08** (`49200f8b`): удалены мёртвые `EarthZero.js` (заменён PresenceLayer, Шаг 3a), `CochlearCylinder.js`, `SmartHologram.js` (Шаг 3b). Осталось: `hologramRenderer.js`, `hologramGridFactory.js`, `TorusVom.js` (нужен до Шага 4/XR) — ⬜ механическая после зелёных 1-7 |
