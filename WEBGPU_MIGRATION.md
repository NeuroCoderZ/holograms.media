...
| 3 | **Портировать слои на граф HoloEngine** — разблокирует удаление `THREE.Scene` | `js/engine/*`, `js/core/init.js` | ✅ **3a DONE** (PresenceLayer), ✅ **3b DONE** (SmartHologram удалён, gestureManager чист); ⬜ 3c: убрать `state.renderer` из рендера слоёв |
