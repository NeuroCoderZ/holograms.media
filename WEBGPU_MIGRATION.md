...
| 3 | **Портировать слои на граф HoloEngine** — разблокирует удаление `THREE.Scene` | `js/engine/*`, `js/core/init.js` | ✅ **3a DONE 10.08** (`7f3d9c4a`): PresenceLayer — нативные маркеры присутствия (замена EarthZero), P2P → `addPresenceMarker`; ⬜ 3b: порт SmartHologram; ⬜ 3c: убрать `state.renderer` из слоёв |
