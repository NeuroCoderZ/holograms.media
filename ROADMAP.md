...
Дата актуализации: 23 мая 2025 г.
Репозиторий: https://github.com/NeuroCoderZ/holograms.media
Ключевые документы-сателлиты: PROJECT_CONTEXT.md, ARCHITECTURE.md, FUTUREARCHITECTURE.md
...
Цель: Создать исчерпывающий MODULE_CATALOG.md, описывающий назначение, зависимости и взаимодействия всех основных модулей фронтенда.

Результат: "Живая карта" фронтенд-кода.

Начало подготовки Backend к PostgreSQL:

Цель: Спроектировать схему БД и подготовить app.py к миграции.

Ключевые задачи:

Детальное проектирование таблиц для "комбинированных аудио(видео)-жестовых чанков", векторных эмбеддингов (pgvector), базы знаний Триа, логов.

Начальный рефакторинг app.py для работы с asyncpg.
...
Полная миграция Backend на PostgreSQL + pgvector:

Цель: Заменить MongoDB на PostgreSQL как основную базу данных.

Ключевые задачи:

Реализация всех CRUD-операций для PostgreSQL.

Настройка pgvector для хранения и поиска векторных эмбеддингов.
...
Актуализация ключевой документации (ARCHITECTURE.md, ROADMAP.md и др.):
