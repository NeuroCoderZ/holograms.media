...
async def sync_knowledge_base():
    ...
    try:
        ...
        print(f"⚠️ Не удалось получить метаданные из AstraDB: {e}. Пропускаем сравнение.")

    print(f"ℹ️  Found {len(remote_files)} files with hash metadata in DB (total docs: {total_docs_in_db}).")
    ...
    if files_to_delete:
        ...
        print(f"   ✅ Deleted {len(files_to_delete)} files from DB.")
    ...
    if files_to_process:
        ...
        for file_path in sorted(files_to_process):
            ...
            for i, chunk_text_val in enumerate(text_chunks):
                ...
                if vector:
                    chunk_id = hashlib.sha256(f"{file_path}:{i}".encode('utf-8')).hexdigest()
                    chunks_to_insert.append({
                        "_id": chunk_id,
                        "content": chunk_text_val[:7900],  # Safety limit для AstraDB
                        "$vector": vector,
                        "metadata": {
                            "source": file_path,
                            "hash": file_hash,
            ...
            if chunks_to_insert:
                try:
                    ...
                    if "DOCUMENT_ALREADY_EXISTS" in err_str or "already exists" in err_str:
