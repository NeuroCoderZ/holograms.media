...
class GeminiEmbeddingService:
    def __init__(self, api_key: str):
        ...
        self.model = "gemini-embedding-2-preview" # ЕДИНСТВЕННО ВЕРНЫЙ ID
    ...
    async def get_embedding(self, text: str, task_type: str = "RETRIEVAL_DOCUMENT") -> Optional[List[float]]:
        ...
        for attempt in range(2):
            try:
                ...
                print(f"⚠️ Попытка {attempt+1}/2: Ошибка Gemini 2: {e}")
...
def chunk_text(text: str, file_path: str, chunk_size: int = CHUNK_SIZE_CHARS, overlap: int = CHUNK_OVERLAP_CHARS) -> List[Dict[str, Any]]:
    ...
    text = re.sub(r'(api[_-]key|token|password|auth|secret|client[_-]secret|private[_-]key|jwt)[:=]\s*([\'"]?)([a-zA-Z0-9_\-\.]{12,})(\2)', 
    ...
    is_sensitive = any(kw in file_path.lower() for kw in ['.env', 'secret', 'config', 'key', 'auth', 'private'])
...
async def sync_knowledge_base():
    print(f"🚀 Starting Smart Sync with Gemini Embedding 2 (ID: gemini-embedding-2-preview)...")
    ...
    files_to_update = {p for p in (set(local_files.keys()) & set(remote_files.keys())) if local_files[p]["hash"] != remote_files[p]}
    ...
    if to_process:
        ...
        for path in sorted(to_process):
            ...
            for idx, chunk_data in enumerate(chunks):
                if total_chunks >= MAX_CHUNKS_PER_RUN: break
